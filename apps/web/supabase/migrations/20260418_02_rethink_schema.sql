-- ============================================================
-- Migration: 20260418_02_rethink_schema.sql
-- Purpose: Phase 2 of 2 for the 2026-04-15 product rethink.
--          New tables, ALTER existing tables, new indexes, RLS, RPCs,
--          seed data. Phase 1 (20260418_01_rethink_enums.sql) must
--          have committed before this runs.
-- Board decisions: docs/product-rethink-2026-04-09/05-BOARD-DECISIONS-2026-04-15.md
-- Audit patches: adds notifications, url_probes, daily_cap_usage tables;
--                fixes C8 plan seed to use discover/build/scale tier values.
...
VALUES
  ('discover', 'Discover', 'discover', 25, 25, 3, 1, 7,
  ...
  ('build', 'Build', 'build', 90, 50, 5, 3, 1,
   ...
   true),
  ('scale', 'Scale', 'scale', 250, 100, 10, 10, 1,
   ARRAY['chatgpt', 'gemini', 'perplexity', 'claude', 'google_ai_overviews', 'grok', 'you_com'],
...
-- ============================================================
-- PART F: Post-audit additions (2026-04-18)
-- Added per DB audit: notifications, url_probes, daily_cap_usage
-- ============================================================


-- F1: notifications — in-app notification center
-- Inserted on every key event (item ready, budget alert, competitor alert, scan complete).
-- Powers sidebar bell dropdown.
...
-- F2: url_probes — off-site verification loop
-- Fires +48h after an archive item's published_at is set.
-- Probes URL for content presence, writes result, updates verification_status
-- on the archive item at the next scheduled scan cycle.
CREATE TABLE IF NOT EXISTS public.url_probes (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  archive_item_id uuid, -- FK target added in a future migration when archive_items table lands
  url text NOT NULL,
  status text NOT NULL DEFAULT 'pending'
    CHECK (status IN ('pending','verified','unverified','failed')),
...
CREATE INDEX IF NOT EXISTS url_probes_user_idx ON public.url_probes(user_id);
CREATE INDEX IF NOT EXISTS url_probes_next_idx
  ON public.url_probes(next_probe_at) WHERE status = 'pending';

ALTER TABLE public.url_probes ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS url_probes_owner ON public.url_probes;
CREATE POLICY url_probes_owner ON public.url_probes
  FOR ALL USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());


-- F3: daily_cap_usage — per-agent daily cap enforcement
-- Checked + incremented by the agent-run middleware for the 4 free agents
-- with daily caps: Schema Generator, FAQ Builder, Off-Site Presence Builder,
-- Performance Tracker. Resets at midnight UTC via the usage_date primary key.
CREATE TABLE IF NOT EXISTS public.daily_cap_usage (
  user_id uuid NOT NULL REFERENCES auth.users(id) ON DELETE CASCADE,
  agent_type agent_type NOT NULL,
  usage_date date NOT NULL DEFAULT current_date,
  count integer NOT NULL DEFAULT 0,
  updated_at timestamptz DEFAULT now(),
  PRIMARY KEY (user_id, agent_type, usage_date)
);

CREATE INDEX IF NOT EXISTS daily_cap_usage_date_idx
  ON public.daily_cap_usage(usage_date);

ALTER TABLE public.daily_cap_usage ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS daily_cap_usage_owner ON public.daily_cap_usage;
CREATE POLICY daily_cap_usage_owner ON public.daily_cap_usage
  FOR ALL USING (user_id = auth.uid())
  WITH CHECK (user_id = auth.uid());
