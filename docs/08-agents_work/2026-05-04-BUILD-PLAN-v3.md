# Beamix MVP — Build Plan v3
Date: 2026-05-04
Author: Build Lead
Status: Supersedes v2 in scope. All v1 ticket IDs (T0.1–T5.6) and v2 ticket IDs (T58–T92) preserved intact. New tickets T93–T133 append in Tier 0–5 order.
...
- Banner copy: "Your agents are ready. Activate them to start fixing your AI search visibility." — Inter 14px
...
### T132 — F50 Public Dogfooding Scan (build at MVP, publish at MVP+30)

**Tier:** 5 (build at MVP; publish/announce at MVP+30)
**Effort:** S (S build + S publishing)
**Dependencies:** T111 (public scan permalink infrastructure), T0.1 (schema — scan data), T90 (State of AI Search — dogfooding scan is editorial context for the report; ship independently)
**References:** Flow Architecture Synthesis §N-6; Q11 decision (build at MVP, publish at MVP+30); F50 (new feature added by v3)

**Acceptance criteria:**
- Beamix's own domain (`beamixai.com`) scanned as a real customer; scan results stored in production DB with a dedicated `scan_id`
- Scan permalink at `/scan/beamixai.com` (or `/scan/[beamix_scan_id]`) publicly accessible without auth — confirms our own AI search visibility
- Editorial framing on the public scan page: a brief Fraunces 300 italic note at top of page: "This is Beamix scanning itself. We publish our own results because we believe in the work." (Inter 15px below) — no marketing copy
- Beamix scan scheduled for weekly re-run (Inngest weekly cron); permalink always shows latest scan
- At MVP+30: Adam publishes the permalink publicly (social post + /changelog entry); page already exists and renders at MVP
- This ticket does NOT include `/scan/beamixai.com` in public navigation (it is a direct-link-only page at MVP)
- `pnpm typecheck` zero errors

**Files touched:**
- `apps/web/src/app/scan/[scan_id]/page.tsx` (editorial framing component — conditional on `is_beamix_scan` flag)
- `apps/web/inngest/functions/beamix-self-scan.ts` (weekly cron)
- `apps/web/supabase/migrations/` (`is_beamix_scan: boolean` column on scans table — OR handled as a dedicated config constant)
...
## Total Ticket Count (v3)

| Tier | v1 tickets | v2 new tickets | v3 new tickets | Total |
|------|-----------|----------------|----------------|-------|
| Tier 0 | 15 (T0.1–T0.15) | 8 (T58–T65) | 7 (T93–T99) | 30 |
| Tier 1 | 8 (T1.1–T1.8) | 5 (T66–T70) | 12 (T100–T111) | 25 |
| Tier 2 | 6 (T2.1–T2.6) | 9 (T71–T79) | 7 (T112–T118) | 22 |
| Tier 3 | 5 (T3.1–T3.5) | 8 (T80–T87) | 10 (T119–T128) | 23 |
| Tier 4 | 3 (T4.1–T4.3) | 0 | 0 | 3 |
| Tier 5 | 6 (T5.1–T5.6) | 5 (T88–T92) | 5 (T129–T133) | 16 |
| **Total** | **43** | **35** | **41** | **119** |

*Note: 119 structured tickets across 6 tiers by ID range T0.1–T133. Ticket IDs are not contiguous (T0.x prefix for v1 Tier 0; T5.x prefix for v1 Tier 5; T1.x–T4.x for v1 product tiers). Total unique ticket IDs: 133 (including v1 alpha-suffixed IDs).*
