...
> **STATUS UPDATE — 2026-08-08, added by session `ceo-1-1786220343` when landing this file to main.**
> Three claims below were true when written and are now stale. Corrected here rather than editing the body, so the original record stands:
> 1. **Item #5 has shipped to main** — merged as PR #192 (`6e28f31`). The body says "committed as `3dafd91` … not yet pushed or PR'd." Nothing is owed on #5's CI half. (The Adam action item — local-session credential scoping — is still open.)
> 2. **All four work branches are pushed to origin** (`fix/hook-compound-command-decomposition` @ `18e637a`, `chore/gh-actions-sha-pinning` @ `ccdd5ce`, `feat/commitlint-enforcement` @ `c8735be`, `feat/spec-conformance-and-qa-lead-accuracy` @ `85ae1f9`). No PRs opened on any of them — parked on the remote, not lost.
> 3. **Item #8's open decision is now made.** Adam chose **scale down to advisory-only**: keep `spec_conformance` as a dimension inside `qa.js`'s findings output, and **drop the `qa-lead-pass.yml` CI frontmatter requirement entirely**. See the `[2026-08-08] — Item #8 scoped down to advisory-only` entry in `.claude/memory/DECISIONS.md` for what that kills and what still needs fixing. Item #8 was **not** executed in that session — the decision is recorded, the rework is still to do.
