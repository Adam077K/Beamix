...
await supabase.rpc('hold_credits', {
  p_user_id: userId,
  p_amount: 1,
  p_job_id: executionId,
})

// 2. Run agent pipeline...

// 3a. SUCCESS: Confirm hold (converts to debit)
await supabase.rpc('confirm_credits', { p_job_id: executionId })

// 3b. FAILURE: Release hold (credits returned)
await supabase.rpc('release_credits', { p_job_id: executionId })
```
...
|------|-----------|--------|
| free_scans | 30 days → delete | Temporary, unauthenticated |
