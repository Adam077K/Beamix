...
**Follow-ups (logged, not blocking):** (1) the $15 T5 cost ceiling is unenforceable — a budget directive isn't CEO-settable on named-workflow calls; needs a real cap mechanism. (2) The "no-override" rule needs a false-positive appeal path — the gate over-blocked twice on FPs (a documented `budget` global; pipeline-contained null-derefs). (3) coding.js worktree-isolated slices need a real integrated-diff ref for qa.js.
