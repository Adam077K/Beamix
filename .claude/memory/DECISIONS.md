...
### [2026-08-08] — Item #8 scoped down to advisory-only; Wave 1 rework gets security review IN the build loop
**Decision (two parts):**
**(a) Item #8 (spec-conformance drift detection) is scaled down to advisory-only.** `spec_conformance` stays as a dimension inside `qa.js`'s findings/blockers output, where a drift finding surfaces to the CEO and to Adam like any other finding. The `.github/workflows/qa-lead-pass.yml` frontmatter requirement (`spec_conformance: PASS` gating merge for `tier: full|irreversible`) is **dropped entirely** — not deferred, dropped. Consequently no session-file migration is needed and CLAUDE.md's Documentation Gate is unchanged. Two defects still must be fixed before the advisory version lands: `deriveSpecConformance` can structurally never return FAIL below P1 (advisory findings are hard-coded `confirmed: false` and never reach the array it checks; sweep-phase findings get relabelled `dimension: 'sweep'`, defeating the dimension check), and the `qa-lead.md` rewrite fails the repo's own `schema-lint.js` while silently dropping that agent's Bash allowlist. Branch `feat/spec-conformance-and-qa-lead-accuracy` @ `85ae1f9` holds the over-scoped version; the rework is not yet done.
**(b) Wave 1's three hardening items are reworked with adversarial security review inside the build loop, not only at the gate.** Slices A/B/C (hook compound-command decomposition, GitHub Actions SHA-pinning, commitlint enforcement) are rebuilt with `security-engineer` + `adversary-engineer` + `code-reviewer` reviewing each slice on its own branch *before* the binding `qa.js` gate ever runs, and a fix round applied from their findings. The prior branches stay on origin as reference; the rework starts fresh from `origin/main` with every confirmed prior defect handed down as an explicit anti-requirement.
**Rationale:** All four items failed the same way — regex/bash/CI security-adjacent logic, built by a single engineer agent, adversarially reviewed only at gate time. Nine QA rounds across Wave 1 plus one on item #8 produced 38 confirmed findings and zero merges; the gate repeatedly caught real defects that both the build agent and the CEO's own manual verification missed (a bash-3.2 crash, a fix relying on a prerequisite set out-of-band, a CI check failing open). Moving the adversary earlier is cheaper than a fourth blind fix round. For item #8 specifically, the CI-enforcement half is where the worst findings clustered (it fails *open* on a tier-line mismatch — the opposite of the adjacent fail-closed check — and its grep is unscoped to the frontmatter block); the drift-detection value survives without it, and a binding gate that the CEO cannot override is the wrong place for a mechanism whose own verification chain never receives the spec it is checking against.
**Decided by:** Adam (CEO session `ceo-1-1786220343`, 2026-08-08, via AskUserQuestion) + CEO (execution design)
**Affects:** `.claude/workflows/qa.js` + `.claude/workflows/lib/gate-logic.mjs` (advisory-only spec-conformance), `.github/workflows/qa-lead-pass.yml` (CI requirement dropped — do not add it), `.claude/agents/qa-lead.md` (accuracy rewrite still owed, must pass schema-lint and keep its Bash allowlist), `.claude/hooks/pre-tool-use.sh` (slice A — also unblocks gap-map items #6 and #7, which were waiting on it), `.github/workflows/**` + `.github/scripts/` (slice B), `scripts/setup-git-hooks.js` + commitlint config (slice C). Gap-map item #9 (local pre-commit gate) stays deferred until slice C resolves.
**Reversible?** Yes — config, hooks, and CI scripts only; no data, schema, or product code touched. Every item is a revertable PR and none is merged without a `qa.js` PASS plus Adam sign-off (all are irreversible-tier by `qa-tier-floor.yml`).
**Housekeeping:** this file is now at 59 entries against the documented ≤50 cap — archiving remains overdue and is still out of scope here.

### [2026-08-09] — Agent-system decisions 15-17: model routing, BLOCKED handling, work arrival
**Status:** proposed
**Decision (three parts, closing the "smaller branches not yet grilled" open item in `docs/08-agents_work/2026-08-09-AGENT-SYSTEM-ARCHITECTURE.md`):**
**(15) Model routing is a compiled tier map.** Agent frontmatter declares a `tier:`; a generator writes the resolved `model:`; `schema-lint.js` fails the build when frontmatter disagrees with the map — same shape as `MANIFEST.json` for skills. Five tiers: `depth` / `default` / `cheap` / `orchestration` / `adversary`, with `adversary: codex` putting decisions 13-14's cross-family routing into **data** rather than prose inside one agent's description. Propagation splits by who holds the knowledge: `.claude/model-routing.yml` universal (tier→ID map + default assignments), `.claude/model-routing.local.yml` project-owned (per-agent overrides, protected by `gsa-sync` fit-precedence). Staleness detection folds into the component-5 weekly reader, which also runs `GET /v1/models` and reports dead pins and available bumps — advisory, gates nothing, and not a new cron (stop condition 7).
**(16) A worker's `BLOCKED` auto-invokes the thinking layer.** The CEO invokes the panel on receipt, records the resolution, and re-dispatches; Adam is not interrupted. Bounded at **two panel runs per `task_id`**, counted by `jq` on the run log — the third block escalates to Adam with both blocker reasons, both panel verdicts, and what changed. The panel may *advise* on an irreversible-tier action but never *authorize* one (decision 9 governs). Adds `task_id` and `status: BLOCKED` (+ `blocker_reason`) to the run-log spec, which previously had only `run_id`, `completed`, and `STALLED`.
**(17) Work arrival is a universal contract with a project-owned rail binding.** Contract: every task carries `task_id`, goal, acceptance criteria (decision 7), and a durable record. Binding lives in `.claude/arrival.local.yml`, validated by `schema-lint.js` — `rail` must resolve to a configured MCP server or the literal `terminal`. Beamix binds Linear **pull-only**: one `.mcp.json` entry, no bridge, no webhook, no HMAC. Rail unavailability degrades gracefully (`status: rail_unavailable` logged, never a hard block).
**Rationale:** All three branches turned out to be the same failure the rebuild exists to end — declared in prose, wired to nothing. Verified this session: `.mcp.json` contains exactly one server (`supabase`) while 23 files call `mcp__linear__*` and `ceo.md`'s step 7 mandates posting a Linear comment; `BLOCKED` is executable only in `coding.js`, which is on the not-being-built list, and the "max 3 retries" limits count nothing; 51 agent files pin literal model IDs while the Opus 4.8 bump recorded 2026-05-28 was never applied and two generations have since shipped. Each decision therefore names a data file plus a resolver, never a convention.
**Deferred, explicitly:** autonomous inbound (a Linear ticket spawning an unattended CEO run) needs the Cloudflare bridge, which is not in this repo. Deferred per stop condition 4 rather than smuggled into an S-sized change, and it competes directly with the QA-gate fix the plan flags as the one item to pull forward regardless of sequence.
**Decided by:** Adam (CEO session `ceo-agent-system-rebuild`, 2026-08-09, five `/grill-me` questions via AskUserQuestion) + CEO (evidence gathering, sub-decisions carried from already-locked doctrine and flagged as carried in the architecture doc)
**Affects:** `docs/08-agents_work/2026-08-09-AGENT-SYSTEM-ARCHITECTURE.md` (decisions 15-17 + 3 mechanism rows), the run-log spec in `2026-08-09-AGENT-SYSTEM-REBUILD-PLAN.md` component 5 (new `task_id` field — the two-panel bound is unimplementable without it), `.claude/hooks/schema-lint.js` (two new lint targets), `.mcp.json` (Beamix `linear` entry), and 51 agent files at generation time.
**Reversible?** Yes — nothing is built. `Status: proposed` is deliberate: no file named above exists on disk yet, which is precisely the claim class decision 8's implemented-claim resolver is designed to catch.
**Housekeeping:** 60 entries against the ≤50 cap. Archiving remains overdue and out of scope.

### [2026-08-09] — Agent-system decisions 18-21: envelope, acceptance criteria, propagation target, run-log consumption
**Status:** proposed
**Decision (four parts, second grill round, same session):**
**(18) The capability envelope is a machine-readable list and `pre-tool-use.sh` detects reaches.** At dispatch the CEO writes `.claude/run/<task_id>/envelope.json` (`skills[]`, `mcps[]`, `tools[]`, `paths[]`); the hook compares every call, appends `{event: 'envelope_reach', tool, task_id, agent}` to the run log when outside it, and exits 0 regardless — advisory, never blocking. Session→task_id resolution reuses the per-session scratch file already spec'd at `hooks.md:315`. The thinking layer proposes the envelope; the CEO owns it and may override.
**(19) Acceptance criteria are `{id, text, check, verified_by: command|judge}`, with asymmetric teeth.** A failed `verified_by: command` criterion is a deterministic BLOCK through the P1-override qa.js already has; a failed `verified_by: judge` criterion enters findings at P1 and must survive the existing 3-verifier adversarial pass first. Run-log fields `ac_total`/`ac_judged`/`ac_failed` make judged-criteria reliability measurable. **This requires plumbing that does not exist:** `qa.js` has zero `args.` references and all five of its dimensions (correctness/security/patterns/tests/perf) are generic code-quality lenses with no task-specific input; `spec_conformance` exists nowhere on main, having died on the unmerged `feat/spec-conformance-and-qa-lead-accuracy` branch.
**(20) Propagation target is the 9 live sibling repos plus Beamix; `gsa-sync` splits into `--check` (new step 3.5) and `--apply` (unmoved, step 9).** `--check` is read-only — per-file sha256 vs canonical plus receiving-project re-classification — so no half-rebuilt state fans out, and it produces the cross-repo inventory decision 12's "useless in every project" cut test requires and which is otherwise unrunnable.
**(21) A reader agent consumes the run log and acts or escalates; Adam leaves the loop.** The weekly reader now carries five signals (STALLED runs, envelope reaches, BLOCKED-rate, judged-criteria reliability, dead model pins), so one unread report blinds all five. **Open hole, explicitly not closed: who checks the reader ran.** Proposed but NOT locked — the reader stamps `last_run` and a far-more-frequent path (CEO session start, or `schema-lint.js` in CI) warns when it goes stale. Awaiting Adam.
**Measurement that replaced two of the plan's five "questions kept open":** cross-project fit was never an open question, only an unmeasured one. 11 repos carry the system. 8 are hand-copied clones of Beamix's 26/144/7/13 baseline (`aiclub`, `beeond`, `etsyc`, `evalove`, `finfun`, `ghostb`, `noam-website`, `realestate`); `adamos` is deliberately divergent at 6 agents / 58 skills but **11 hooks to Beamix's 7** — a sibling is ahead, making fit-precedence load-bearing on the first `--apply` rather than theoretical; `hitstampjavagame`/`ml2`/`test1` carry the pre-cleanup 426-574-skill corpus, no `qa-tier-floor.yml`, and **zero commits in 90 days**, so they are out of scope by data. **The ROI thesis was understated, not overstated:** `etsyc` (689 commits/90d) and `evalove` (385, committed 2026-08-09) both out-commit Beamix.
**Also this session (Adam's request):** 24 reference agent/skill systems cloned shallow to `~/VibeCoding/_reference/` (406M, 1,150 `SKILL.md` files vs Beamix's 146) — inventory and study guide at `docs/08-agents_work/2026-08-09-reference-systems-local.md`. Headline: **hooks are where Beamix is furthest behind on the one axis its own doctrine calls load-bearing** — Beamix 7, `open-gsd/gsd-core` 78, `gsd-build/get-shit-done` 54, `cloudflare/agents` 41. This repo archived GSD's *agents* on 2026-05-16 and never took its hook library. Two assumptions corrected by looking: `yc-software/qm` has zero agents and zero hooks (a skill-*format* innovation, not an agent org — decision 6 was right to copy only the `requiredCapabilities` mechanism), and `buildermethods/agent-os` ships 22 files with no roster, confirming the 2026-08-08 finding. All 24 are third-party content and therefore the `provenance: untrusted` case component 1 exists for: reading is free, copying into `.claude/skills/` or `.claude/memory/` stays gated.
**Decided by:** Adam (CEO session `ceo-agent-system-rebuild`, 2026-08-09, four `/grill-me` questions via AskUserQuestion) + CEO (measurement, sub-decisions carried from locked doctrine and flagged as carried)
**Affects:** `docs/08-agents_work/2026-08-09-AGENT-SYSTEM-ARCHITECTURE.md` (decisions 18-21 + detail), `2026-08-09-AGENT-SYSTEM-REBUILD-PLAN.md` (new step 3.5; cross-project-fit open question closed with data), `.claude/hooks/pre-tool-use.sh` (envelope comparison), `.claude/workflows/qa.js` (first task-specific args surface + acceptance dimension), `scripts/gsa-sync` (`--check` mode), the run-log spec (`ac_*` fields, `envelope_reach` events).
**Reversible?** Yes — nothing built. `Status: proposed` is deliberate: none of these files exist yet, which is the claim class decision 8's implemented-claim resolver is built to catch.
**Housekeeping:** 61 entries against the ≤50 cap.

### [2026-08-11] — Agent-system decisions 24-26: envelope deny-path, inbound guards, cut-pairing retired
**Status:** proposed
**Decision (three parts — the last open branches, all three conflicts between the architecture doc and the 2026-08-08 capability gap map, both of which were decided with Adam):**
**(24) The envelope ships advisory; its default-deny path is built and left OFF behind one flag.** Decision 18 chose advisory-and-logged; gap-map rec 6 chose per-skill default-deny. Both were live decisions for one mechanism and only one can ship. Advisory wins on **ordering, not merit** — a blocked reach is a reach nobody sees, so deny-first destroys the exact signal decision 4 calls the highest-value data the system can collect. Writing the deny branch now and disabling it costs one flag, prevents rec 6 being re-argued from zero later, and makes the eventual switch a config change once the run log shows what it would have caught. Lands in build step 4c.
**(25) Inbound guards enter the build — gap-map recs 4 and 7 only.** Every other guard in this design is **outbound** (what an agent may do); nothing inspects what comes **in**, while the scan pipeline ingests third-party web pages and four LLM providers' output directly into agent context and 24 untrusted repos now sit at `~/VibeCoding/_reference/`. Rec 4 (PostToolUse injection + invisible-unicode scan of Read/WebFetch/MCP output) already has a selected port — `read-injection-scanner`, present in **both** external hook libraries. Rec 7 (secret redaction from tool output before the model sees it) has **no prior art found** by the hook audit and is genuinely ours to build; keep it narrow to known key shapes. Rec 5 (credential scoping) stays out — half-covered by locked decision 4 on global permissions. The other eight recs stay on the not-building list. Lands as new build step 4.5.
**(26) The 2026-08-08 net-zero cut-pairing policy is retired.** It required every new-file recommendation to pair with a verified skill cut. This build adds ~15 files, so binding it would force step 13 — the one cut nobody could prove is quality-neutral, and deliberately sequenced last for that reason — to grow by the same amount on a schedule set by unrelated work. `stripe-integration` and `clerk-auth` remain agreed cuts; everything else earns its cut against decision 12's "useless in every project" test or stays.
**Correction recorded, not silently dropped:** the build handoff had also recommended folding in gap-map rec 3 (commit-message enforcement). That contradicted two prior decisions — the rebuild plan **cut** commitlint for breaking 50+ worktrees, and the hook audit explicitly refused to port the commit-format validator. Rec 3 stays cut; the recommendation was wrong and is corrected in place in the handoff rather than deleted.
**Finding that changed the build order:** open PR #189 (`feat/gsa-core-sync`, 2026-07-27, still MERGEABLE) was read this session to answer the handoff's own instruction *"read that diff before building steps 2/4/5."* Answer: **the build is not already written there.** Per-file, its hook and workflow edits are version stamps (`pre-tool-use.sh` +2/-2, `schema-lint.js` +1/-1, `stop.sh` +1/-1, `qa.js` +5/-5, `gate-logic.mjs` +1/-1). What it does carry: `gsa-project.json` (the file step 12a says no project has — it exists here as a draft to finish, not re-author); a **duplicate-gate hazard**, since it adds a new 343-line `.github/workflows/gsa-qa-lead-pass.yml` *alongside* the existing `qa-lead-pass.yml` and a `gsa-promptfoo-eval.yml` alongside `promptfoo-eval.yml` — the sync tool installs `gsa-`-prefixed copies rather than overwriting, so landing it would leave two QA gates live and step 5 would harden only one; and a CLAUDE.md restructure into `@.claude/AGENT-SYSTEM.md`, checked and found to be an extraction rather than a fit-clobber (Framer 8/8, `apps/web` 4/4, Project State intact; Paddle 3→2 and Supabase 4→3 lost one mention each) — but its merge base is `deabafd`, six weeks stale, and GitHub's MERGEABLE means no textual conflict, not that six weeks of changes survive.
**Decided by:** Adam (CEO session `ceo-agent-system-rebuild`, 2026-08-11, four questions via AskUserQuestion — the fourth restored a working-copy revert of the build handoff that had silently dropped 109 lines including three verified numeric corrections) + CEO (PR #189 inspection, rec-3 contradiction)
**Affects:** `docs/08-agents_work/2026-08-10-BUILD-HANDOFF.md` (steps 4c, new 4.5, 5f, 12c, 13; UNRECONCILED section closed), `docs/08-agents_work/2026-08-09-AGENT-SYSTEM-ARCHITECTURE.md` (decisions 24-26 + detail; heading corrected 14→26), and at build time `.claude/hooks/pre-tool-use.sh` (deny branch, flag-gated) plus one new PostToolUse hook (recs 4 and 7).
**Reversible?** Yes — nothing is built. `Status: proposed` is deliberate and load-bearing: no file named above exists on disk, which is exactly the claim class decision 8's implemented-claim resolver is built to catch.
**Housekeeping:** 62 entries against the ≤50 cap. Archiving is now 12 entries overdue and remains out of scope here — the `write-guard` port selected by the hook audit is the mechanism that would make this cap real rather than noted.
