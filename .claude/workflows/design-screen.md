# design-screen — Beamix Design Operating System Workflow

The ultracode orchestration script for the Beamix design pipeline. One screen in, one
...
2. Runs the dedicated front-end **product-designer** to BUILD the screen in Beamix's own
   design language.
...
export const meta = {
  name: "design-screen",
  description:
    "Beamix design pipeline as a deterministic Workflow: load the screen's reference folder + global product-feel, run the product-designer build, then a design-critic <-> design-polisher loop until craft-parity with the references (expressed in Beamix's own language), and return the validated screen + screenshots. References are vibe, not blueprint. Three founder checkpoints (lock refs, 50% first-paint, judge final) are surfaced as stop points.",
  phases: [
    ...
    { title: "Build", detail: "product-designer builds the screen in Beamix's own design language." },
...
const reference = await agent(
  `You are the design-lead assembling the reference contract for the Beamix screen "${screen}".
...
Return JSON ONLY:
{
  ...
  "feeling_brief": "2-4 sentences: the richness/confidence/polish to hit, expressed for Beamix",
  "direction": "the DIRECTION for the build: the one memorable element, the primary layout move, and the motion-budget tier (1/2/3) — expressed for Beamix",
...
// ---------------------------------------------------------------------------
// PHASE 2 — BUILD  (product-designer builds in Beamix's own language)
// ---------------------------------------------------------------------------
phase("Build");
log(`product-designer building "${screen}".`);

const build = await agent(
  `You are the dedicated front-end product-designer building the Beamix "${screen}" screen.
...
ORIGINAL in Beamix's own design language. Inspired-by, never traced. Do NOT reproduce a
...
beamix-brand-quality-bar (AUTHORITATIVE — Inter/InterDisplay/Fraunces/Geist Mono + #3370FF palette,
8pt grid, rounded-lg product utility), frontend-design, humanizer (all copy), full-output-enforcement
(zero stubs/TODOs). Apply the generic skills' techniques with Beamix's locked brand tokens.
...
while (round < MAX_ROUNDS) {
  ...
  const critic = await agent(
    `You are the design-critic grading the Beamix "${screen}" build against its reference folder.
...
expressed as Beamix?" PASS = indistinguishable in CRAFT-LEVEL from the references, in Beamix's own
language — not a pixel match.

Load ui-visual-validator + beamix-brand-quality-bar. Read all of "${productFeelDir}" and all of
...
considered detail, brand-token discipline (#3370FF, Inter/InterDisplay/Fraunces/Geist Mono, 8pt grid,
all four states).

Return a SPECIFIC "here is what's missing to reach the references' craft bar, expressed as Beamix"
...
Return JSON ONLY:
{
  ...
  if (critic && critic.verdict === "PASS") {
    log(`Critic PASS on round ${round}. Craft-parity reached (in Beamix's language).`);
    break;
  }
  ...
  const polish = await agent(
    `You are the design-polisher. Your SOLE job is adding CRAFT DENSITY to the Beamix "${screen}" build
...
Load beamix-brand-quality-bar (AUTHORITATIVE tokens) + emilkowal-animations + design-taste-frontend +
...
transform/opacity, ease-out entering, 200-400ms UI, prefers-reduced-motion fallback). Stay in Beamix's
