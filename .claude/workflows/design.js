export const meta = {
  name: 'design',
  description: 'Beamix T5 design workflow — generates N independent design variations from different angles, scores each with parallel design-critic judges against the brand bar, and synthesizes a winning direction (grafting the best ideas from runners-up). Optimizes for craft/quality, not speed.',
  phases: [
...
const ANGLES = [
  ...
  'bold / brand-forward — lean hardest into the Beamix accent (#3370FF) and motion budget',
...
const SCORE_SCHEMA = {
  ...
  properties: {
    brand_fidelity: { type: 'number', description: '0-10 vs Beamix brand bar (color, type, spacing, motion)' },
...
Honor the Beamix brand bar (load the beamix-brand-quality-bar skill): accent #3370FF, Inter/InterDisplay + Fraunces + Geist Mono, intentional spacing, restrained motion, zero placeholder UI, all empty/loading/error/success states considered. Return concept, layout, rationale, risks.`
}
...
const synthesis = await agent(
  `Synthesize the final Beamix design direction for: ${TARGET}.
Brief: ${BRIEF}
